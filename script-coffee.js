...
supprimerCafe(idCafe) {
    fetch(`https://insta-api-api.0vxq7h.easypanel.host/coffees/${idCafe}`, {
        method: "DELETE"
    })
    .then(response => {
        if (response.ok) {
            const cafeSupprime = document.querySelector(`#cafe-list li[cafe-class="${idCafe}"]`);
            if (cafeSupprime) {
                cafeSupprime.remove();
            }
        }
